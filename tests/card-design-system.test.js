...
B.moveModule(template,'title',399,359);assert.equal(template.modules[0].x,240,'movement clamps to card width');assert.equal(template.modules[0].y,320,'movement clamps to card height');
...
B.setCanvasPreset(template,'small');assert.equal(template.canvas.width,320);assert.equal(template.canvas.height,320);template.modules.forEach(module=>{assert.ok(module.x>=0&&module.y>=0);assert.ok(module.x+module.width<=320);assert.ok(module.y+module.height<=320);});
...
standard.id='standard-test';state.visualTemplates.push(standard);state.books[0].visualTemplateId=standard.id;const assigned=B.templateForBook(state.books[0]),canvas=B.renderTemplateCanvas(assigned,state.books[0]),card=B.renderBookCard(state.books[0]);assert.match(card,/data-template-id="standard-test"/,'the final card uses the assigned visual template');assert.match(canvas,/data-layout-mode="absolute"/);
assert.match(source,/renderTemplateCanvas\(editorDraft,record,\{editor:true,size:previewSize,layoutMode:/,'builder calls shared renderer');assert.match(source,/renderTemplateCanvas\(template,book,\{visible,presentation,size\}\)/,'library card calls shared renderer');
