...
function setAssessmentMessage(rankIndex: number, onboarded: boolean) {
  ...
  if (!onboarded) {
    ...
    detail.textContent = 'Complete the court assessment to begin revealing the identity your court recognizes in you.';
    return;
  }

  if (rankIndex < 1) {
    heading.textContent = 'Fae role revealed';
    detail.textContent = `Your primary gift remains sealed until ${PRYTHIAN_RANKS[1]}.`;
  } else if (rankIndex < 2) {
    heading.textContent = 'Primary gift revealed';
    detail.textContent = `Your rare affinity remains sealed until ${PRYTHIAN_RANKS[2]}.`;
  } else {
...
function applyRevealGates() {
  ...
  if (!onboarded && role) lockedCard(role, 'Fae role', 0);
  if ((!onboarded || rankIndex < 1) && gift) lockedCard(gift, 'Primary gift', 1);
  ...
  if (!onboarded || rankIndex < 2) {
    lockedCard(affinity, 'Rare affinity', 2);
