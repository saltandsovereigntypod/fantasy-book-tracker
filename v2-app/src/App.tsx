...
export default function App() {
  ...
  function beginInteraction() {
    interactionStartRef.current = cloneDesign(designRef.current);
  }
        ...
        <aside className="panel inspector-panel">
          ...
          {selectedElement ? <div className="field-stack inspector-stack">
            ...
            <section className="inspector-group"><h3>Appearance</h3>
              ...
              {selectedElement.type === 'shape' && <><label>Fill<input type="color" value={selectedElement.fill} onChange={(event) => updateSelected({ fill: event.target.value } as Partial<DesignElement>)} /></label><label>Border<input type="color" value={selectedElement.stroke ?? '#75451f'} onChange={(event) => updateSelected({ stroke: event.target.value } as Partial<DesignElement>)} /></label></>}
              ...
              {selectedElement.type === 'image' && <label>Image fit<select value={selectedElement.fit ?? 'cover'} onChange={(event) => updateSelected({ fit: event.target.value as 'cover' | 'contain' } as Partial<DesignElement>)}><option value="cover">Crop to fill</option><option value="contain">Fit inside</option></select></label>}
            </section>

            <section className="inspector-group"><h3>Layers and object</h3><div className="quick-action-grid">
              <button onClick={() => moveSelectedLayer('back')}>To back</button><button onClick={() => moveSelectedLayer('backward')}>Backward</button><button onClick={() => moveSelectedLayer('forward')}>Forward</button><button onClick={() => moveSelectedLayer('front')}>To front</button><button onClick={duplicateSelected}>Duplicate</button><button className={selectedElement.locked ? 'is-active' : ''} onClick={() => updateSelected({ locked: !selectedElement.locked })}>{selectedElement.locked ? 'Unlock' : 'Lock'}</button>
            </div></section>
            <button className="danger-button" onClick={removeSelected}>Remove from design</button>
          </div> : <p className="muted-copy">Select an element on the card or use an “On card” button in the Book panel.</p>}
        </aside>
      </main>
    </div>
  );
