...
const legacy=B.normalizeTemplate({id:'legacy',schemaVersion:2,canvas:{width:420,height:360},modules:[{id:'old',type:'text',groupId:'old-group'}]});assert.equal(legacy.schemaVersion,6);assert.equal(legacy.groups[0].id,'old-group');assert.equal(legacy.modules[0].anchor.targetType,'none');assert.equal(legacy.modules[0].responsive.scale,'proportional');assert.equal(legacy.scalingRules.positions,'proportional');assert.deepEqual(B.normalizeTemplate(legacy),legacy,'schema migration is idempotent');
const standard=B.standardBookTemplate();for(const group of ['Identity','Progress','Ratings','Actions'])assert.ok(standard.groups.some(item=>item.name===group),group);assert.ok(standard.modules.every(module=>module.responsive.mode==='absolute'));assert.equal(standard.composer.enabled,false);
...
for(const hook of ['data-editing-mode="freeform"','data-editing-mode="expert"','data-inspect-scaling-curve','data-preview-mode'])assert.ok(source.includes(hook),hook);
