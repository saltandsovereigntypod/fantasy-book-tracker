...
export function CardActionsPreview({ actions, size, interactive = false, onAction }: { actions: CardAction[] | undefined; size: CardSize; interactive?: boolean; onAction?: (action: CardAction) => void }) {
  ...
  const outputWidth = CARD_WIDTHS[size]; const scale = outputWidth / 420; const outputHeight = 380 * scale;
  return <div className="v2-runtime-actions v2-runtime-actions--overlay" style={{ width: outputWidth, height: outputHeight }} aria-label="Card actions">
    {visible.map((action) => <button key={action.id} type="button" disabled={!interactive} data-action-type={action.action} onClick={(event) => { event.stopPropagation(); if (interactive) onAction?.(action); }} style={{ left: action.x * scale, top: action.y * scale, width: action.width * scale, height: action.height * scale, background: action.background, color: action.color, border: `${Math.max(1, scale)}px solid ${action.borderColor}`, borderRadius: action.borderRadius * scale, fontFamily: action.fontFamily, fontSize: action.fontSize * scale, fontWeight: action.fontWeight, textAlign: action.textAlign, justifyContent: action.textAlign === 'left' ? 'flex-start' : action.textAlign === 'right' ? 'flex-end' : 'center', padding: `${4 * scale}px ${7 * scale}px` }}>{action.icon && <span aria-hidden="true">{action.icon}</span>}{action.label}</button>)}
  </div>;
}
