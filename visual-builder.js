(() => {
  ...
  function activeTemplate() { return editorDraft; }
  function beginDraft(template,{asNew=false,name='',id=''}={}){const source=template||createTemplate({name:name||'Untitled Visual Template'});editorDraft=normalizeTemplate({...JSON.parse(JSON.stringify(source)),...(asNew?{id:id||makeId('template'),name:name||source.name,createdAt:Date.now(),updatedAt:Date.now()}: {})});editorOriginalId=asNew?'':source.id;editorIsNew=asNew;activeTemplateId=editorDraft.id;selectedModuleIds.clear();histories.set(editorDraft.id,[]);return editorDraft;}
  function discardDraft(){editorDraft=null;editorOriginalId='';editorIsNew=false;interaction=null;selectedModuleIds.clear();}
  function commitDraft(){if(!editorDraft)return null;const committed=normalizeTemplate({...JSON.parse(JSON.stringify(editorDraft)),updatedAt:Date.now()}),index=state.visualTemplates.findIndex(item=>item.id===(editorOriginalId||committed.id));if(index>=0)state.visualTemplates[index]=committed;else state.visualTemplates.push(committed);if(builderContext.recordType==='book'){const book=state.books.find(item=>item.id===builderContext.recordId);if(book)book.visualTemplateId=committed.id;}editorDraft=normalizeTemplate(JSON.parse(JSON.stringify(committed)));editorOriginalId=committed.id;editorIsNew=false;activeTemplateId=committed.id;persist();return committed;}
  ...
  function undo(template) { const stack=histories.get(template.id)||[];const previous=stack.pop();if(!previous)return false;editorDraft=normalizeTemplate(JSON.parse(previous));activeTemplateId=editorDraft.id;return true; }
  ...
  function openForBook(bookId){const book=state.books.find(item=>item.id===bookId);if(!book)return;const fallback=ensureDefaultBookTemplate();let template=state.visualTemplates.find(item=>item.id===book.visualTemplateId),draftOptions={};if(!template||template.id===DEFAULT_BOOK_TEMPLATE_ID){template=fallback.template;draftOptions={asNew:true,name:`${book.title} Card`};}open(template.id,{recordType:'book',recordId:book.id,draftOptions});}
  ...
  function bind(root,template) {
    ...
    root.querySelector('[data-lock-module]')?.addEventListener('click',()=>{snapshot(template);lockModule(template,selected().id,!selected().locked);refresh();});
    root.querySelector('[data-duplicate-module]')?.addEventListener('click',()=>{snapshot(template);const copy=duplicateModule(template,selected().id);selectedModuleIds=new Set([copy.id]);refresh();});
    root.querySelector('[data-delete-module]')?.addEventListener('click',()=>{snapshot(template);deleteModule(template,selected().id);selectedModuleIds.clear();refresh();});
  }
  ...
  function open(templateId = '',context=null) {
    ...
    ensureState();
    let template=state.visualTemplates.find(item=>item.id===templateId)||state.visualTemplates[0],draftOptions=context?.draftOptions||{};
    if(!template){template=createTemplate({name:'My Visual Template'});draftOptions={asNew:true,name:template.name};}
    beginDraft(template,draftOptions);
    if(modal('<section id="visualBuilderRoot" class="visual-builder" aria-label="Universal Visual Builder"></section>')===false){discardDraft();return false;}
    const host=document.getElementById('formModal');if(!host){discardDraft();return false;}host.classList.add('visual-builder-backdrop');
    render();return true;
  }
