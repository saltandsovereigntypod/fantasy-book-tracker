(() => {
  ...
  let saveTimer;
  let frame;
  let wallFilters = {query:'',category:'',tag:'',book:'',status:'',region:'',unresolved:false,orphans:false};
  ...
  function defaultTileSettings(card = {}) {
    return {mode:card.displayMode || 'standard',showSummary:true,showImage:true,showCounts:true,showTags:true,showCategory:true};
  }
  ...
  function runAutomaticAssignments() {
    ...
    activeRegions().filter(r=>r.assignmentMode==='automatic').forEach(region => matchingDossiers(region).forEach(dossier => {
      if (state.wallAppearances.some(a=>a.wallId===region.wallId&&a.regionId===region.id&&a.dossierId===dossier.id)) return;
      const existingHome=state.wallAppearances.some(a=>a.wallId===region.wallId&&a.dossierId===dossier.id&&a.appearanceType==='home');
      state.wallAppearances.push({id:uid(),dossierId:dossier.id,wallId:region.wallId,regionId:region.id,appearanceType:existingHome?'reference':'home',x:region.x+30,y:region.y+90,width:300,height:null,displayMode:'standard',zIndex:10,locked:false});
      changed = true;
    }));
  ...
  function filterAppearances(items) {
    return items.filter(item => { const d=dossierById(item.dossierId); if(!d)return false;
      if(wallFilters.query&&!`${d.title} ${d.summary} ${(d.tags||[]).join(' ')} ${(d.fields||[]).map(fieldValue).join(' ')}`.toLowerCase().includes(wallFilters.query.toLowerCase()))return false;
      ...
      if(wallFilters.book&&d.sourceBookId!==wallFilters.book)return false;
  ...
  function bindInfiniteWall() {
    ...
    elBind('[data-filter-unresolved]',()=>{wallFilters.unresolved=!wallFilters.unresolved;renderWall();});
    document.querySelectorAll('[data-zoom]').forEach(b=>b.onclick=()=>zoomAt(b.dataset.zoom==='in'?1.2:1/1.2));
    ['wallCategory','wallTag','wallBook','wallRegion'].forEach(id=>document.getElementById(id).onchange=e=>{wallFilters[{wallCategory:'category',wallTag:'tag',wallBook:'book',wallRegion:'region'}[id]]=e.target.value;renderWall();});
  ...
  function assignToContainingRegion(a){const cx=a.x+(a.width||300)/2,cy=a.y+(a.height||150)/2;const region=activeRegions().filter(r=>!r.collapsed&&cx>=r.x&&cx<=r.x+r.width&&cy>=r.y&&cy<=r.y+r.height).sort((a,b)=>(b.zIndex||1)-(a.zIndex||1))[0];a.regionId=region?.id||'';}
  ...
  function ruleRow(rule={}){return `<div class="rule-row" data-rule-id="${rule.id||uid()}"><input class="rule-key" value="${esc(rule.key||'category')}" placeholder="category or field:Status"><select class="rule-operator">${['equals','notEquals','oneOf','contains','gt','lt','hasLinks','noLinks'].map(x=>`<option value="${x}" ${rule.operator===x?'selected':''}>${x}</option>`).join('')}</select><input class="rule-value" value="${esc(rule.value||'')}" placeholder="Match value"><button type="button" data-remove-rule>×</button></div>`;}
  ...
  function acceptSuggestions(id){const r=state.wallRegions.find(x=>x.id===id);matchingDossiers(r).forEach(d=>{if(!state.wallAppearances.some(a=>a.wallId===r.wallId&&a.regionId===r.id&&a.dossierId===d.id)){const home=state.wallAppearances.some(a=>a.wallId===r.wallId&&a.dossierId===d.id&&a.appearanceType==='home');state.wallAppearances.push({id:uid(),dossierId:d.id,wallId:r.wallId,regionId:r.id,appearanceType:home?'reference':'home',x:r.x+30,y:r.y+90,width:300,height:null,displayMode:'standard',zIndex:10,locked:false});}});saveState();renderWall();}
