...
import { freshArchive, loadCloudArchive, saveCloudArchive, type V2ArchiveState } from './archive';
...
export default function FullApp() {
  ...
  return <div className="v2-full-app">
    <aside className="v2-app-sidebar">
      ...
      <nav>{NAV_ITEMS.map((item) => <button key={item.id} className={view === item.id ? 'is-active' : ''} onClick={() => setView(item.id)}><span>{item.icon}</span>{item.label}</button>)}</nav>
    ...
    <main className="v2-app-main">
      ...
      <section className={`v2-view v2-view--${view}`}>
        {view === 'dashboard' && <Dashboard archive={archive} onNavigate={setView} />}
        {view === 'library' && <Library archive={archive} onEdit={() => setView('editor')} />}
...
function Dashboard({ archive, onNavigate }: { archive: V2ArchiveState; onNavigate: (view: AppView) => void }) {
  ...
  return <div className="v2-dashboard"><section className="v2-hero"><div><p>Private Reading Command</p><h2>Every chapter leaves evidence.</h2><span>Track the books, preserve the theories, and build the card exactly the way you want it.</span><div><button onClick={() => onNavigate('editor')}>Add or Design a Book</button><button onClick={() => onNavigate('library')}>Open Library</button></div></div><strong>✦</strong></section><div className="v2-metric-grid"><article><span>Total Books</span><strong>{archive.books.length}</strong></article><article><span>Currently Reading</span><strong>{activeBooks.length}</strong></article><article><span>Completed</span><strong>{completed}</strong></article><article><span>Open Theories</span><strong>{archive.theories.length}</strong></article></div><section className="v2-dashboard-panel"><header><h3>Active Reading</h3><button onClick={() => onNavigate('library')}>View all</button></header>{activeBooks.length ? <div className="v2-mini-books">{activeBooks.slice(0, 3).map((book) => <article key={book.id}><strong>{book.title}</strong><span>{book.author}</span><small>{book.progress}% complete</small></article>)}</div> : <p>No active books yet. The editor is ready when you are.</p>}</section></div>;
}

function Library({ archive, onEdit }: { archive: V2ArchiveState; onEdit: () => void }) {
  ...
  return <div className="v2-library"><header><div><h2>Book Library</h2><p>The same renderer used in the editor powers every saved card.</p></div><button onClick={onEdit}>Add Book</button></header><div className="v2-library-controls"><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search books" /><select value={size} onChange={(event) => setSize(event.target.value as CardSize)}><option value="small">Small cards</option><option value="medium">Medium cards</option><option value="large">Large cards</option></select></div>{books.length ? <div className={`v2-library-grid is-${size}`}>{books.map((book) => <article key={book.id}><CardRenderer book={book} design={book.design} size={size} /><footer><strong>{book.title}</strong><button onClick={onEdit}>Edit</button></footer></article>)}</div> : <div className="v2-empty-state"><span>▤</span><h3>No books found</h3><p>Create your first book in the V2 workspace.</p><button onClick={onEdit}>Open Workspace</button></div>}</div>;
}
