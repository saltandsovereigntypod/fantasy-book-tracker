...
export default function CoreFullApp() {
  ...
  const rank = path.ranks[rankIndex];
  ...
  const themeStyle = {
    '--v2-bg': path.theme.background, '--v2-panel': path.theme.panel, '--v2-panel-raised': path.theme.panelAlt,
    '--v2-border': path.theme.border, '--v2-border-strong': path.theme.accent, '--v2-text': path.theme.text,
    '--v2-muted': path.theme.muted, '--v2-accent': path.theme.accent, '--v2-accent-bright': path.theme.accent,
    '--path-background': path.theme.background, '--path-panel': path.theme.panel, '--path-panel-alt': path.theme.panelAlt,
    '--path-surface': path.theme.surface, '--path-text': path.theme.text, '--path-muted': path.theme.muted,
    '--path-accent': path.theme.accent, '--path-accent-soft': path.theme.accentSoft, '--path-border': path.theme.border,
    '--path-paper': path.theme.paper,
  } as CSSProperties;

  return <div className="v2-full-app core-path-app" data-path={path.id} style={themeStyle}>
    <aside className="v2-app-sidebar"><div className="v2-brand"><span>{path.glyph}</span><div><small>The Empyrean Tracker</small><strong>{path.short}</strong></div></div><nav>{navItems.map((item) => <button key={item.id} className={view === item.id ? 'is-active' : ''} onClick={() => { setProfileBookId(null); setView(item.id); }}><span>{item.icon}</span>{item.label}</button>)}</nav><div className="v2-sidebar-footer"><small>{rank}</small><strong>{displayName}</strong><button onClick={() => signOut().catch((reason) => setError(reason.message))}>Sign Out</button></div></aside>
    <main className="v2-app-main"><header className="v2-topbar"><div>{view === 'editor' && <button className="v2-editor-back" onClick={() => setView('library')}>← Back to {path.copy.navLibrary}</button>}<p>{viewLabel}</p><h1>{view === 'editor' ? 'Book Workspace' : `Welcome back, ${displayName}`}</h1></div><span className={`v2-sync-state is-${syncState}`}>{syncState === 'saving' ? 'Saving…' : syncState === 'saved' ? 'Cloud saved' : syncState === 'error' ? 'Save failed' : 'Cloud ready'}</span></header>
      ...
      <section className={`v2-view v2-view--${view}`}>
        {view === 'dashboard' && <Dashboard archive={archive} pathId={path.id} onNavigate={setView} onNewBook={openNewBook} onProfile={setProfileBookId} />}
        {view === 'library' && <Library archive={archive} title={path.copy.navLibrary} addLabel={path.copy.addBook} onNewBook={openNewBook} onEditBook={openBook} onProfile={setProfileBookId} onDeleteBooks={deleteBooks} onSaveBooks={saveBooks} />}
...
function Dashboard({ archive, pathId, onNavigate, onNewBook, onProfile }: { archive: V2ArchiveState; pathId: PathId; onNavigate: (view: AppView) => void; onNewBook: () => Promise<void>; onProfile: (id: string) => void }) {
  const path = PATHS[pathId]; const rank = path.ranks[rankIndexForPoints(pathId, archive.profile.points || 0)];
  ...
  return <div className="v2-dashboard"><section className="core-assignment-compact"><div><span>Your assignment</span><strong>{path.glyph} {path.name}</strong></div><div><span>{path.copy.currentRank}</span><strong>{rank}</strong><small>{archive.profile.points || 0} points</small></div></section><section className="v2-hero"><div><p>{path.short}</p><h2>{path.copy.heroTitle}</h2><span>{path.copy.heroBody}</span><div><button onClick={() => onNewBook()}>{path.copy.addBook}</button><button onClick={() => onNavigate('library')}>Open {path.copy.navLibrary}</button></div></div><strong>{path.glyph}</strong></section><div className="v2-metric-grid"><article><span>Total Books</span><strong>{archive.books.filter((book) => !book.archived).length}</strong></article><article><span>Currently Reading</span><strong>{activeBooks.length}</strong></article><article><span>Completed</span><strong>{completed}</strong></article><article><span>Open Theories</span><strong>{archive.theories.filter((theory) => theory.status === 'open').length}</strong></article></div><section className="v2-dashboard-panel"><header><h3>Active Reading</h3><button onClick={() => onNavigate('library')}>View all</button></header>{activeBooks.length ? <div className="v2-mini-books">{activeBooks.slice(0,3).map((book) => <button type="button" key={book.id} onClick={() => onProfile(book.id)}><strong>{book.title}</strong><span>{book.author || 'Unknown author'}</span><small>{book.progress}% complete</small></button>)}</div> : <p>No active books yet.</p>}</section></div>;
}

function Library({ archive, title, addLabel, onNewBook, onEditBook, onProfile, onDeleteBooks, onSaveBooks }: { archive: V2ArchiveState; title: string; addLabel: string; onNewBook: () => Promise<void>; onEditBook: (id: string) => Promise<void>; onProfile: (id: string) => void; onDeleteBooks: (ids: string[]) => Promise<void>; onSaveBooks: (books: V2BookRecord[]) => Promise<void> }) {
