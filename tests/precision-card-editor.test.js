...
for(const hook of ['beginDirectCardInteraction','moveWithVisibleBounds','Saved to this card','Arrange neatly','data-module-look','translate3d','Math.hypot(event.clientX-interaction.startX'])assert.ok(source.includes(hook),hook);
for(const style of ['data-module-presentation=pill','data-module-presentation=tag','data-module-presentation=button','data-module-presentation=card','data-module-presentation=soft-pill','data-module-presentation=solid-button','data-module-presentation=accent-card','builder-save-status','builder-resize-handle.is-n','builder-look-picker','overflow-y:scroll','scrollbar-gutter:stable'])assert.ok(css.includes(style),style);
