...
B.setCanvasPreset(template,'small');assert.equal(template.canvas.width,320);assert.equal(template.canvas.height,220);assert.ok(title.x+title.width<=320&&title.y+title.height<=220);
...
for(const hook of ['data-editing-mode="smart"','data-editing-mode="expert"','data-anchor-type','data-inspect-scaling-curve','data-preview-mode'])assert.ok(source.includes(hook),hook);
