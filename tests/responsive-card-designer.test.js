...
const custom=B.createTemplate({id:'freeform',groups:[{id:'free',name:'Freeform',layout:'freeform'}],modules:[B.createModule('text',{id:'custom',groupId:'free',x:73,y:91,width:177,height:83,responsive:{mode:'absolute'}})]});const customMarkup=B.renderTemplateCanvas(custom,state.books[0],{size:'small'});assert.match(customMarkup,/data-layout-mode="absolute"/);assert.match(customMarkup,/--module-x:/,'custom expert coordinates remain available');
assert.match(source,/editingMode==='smart'\?'responsive':'absolute'/);assert.match(source,/target\.responsive\.mode='absolute'/,'dragging out of a smart container preserves freeform design');assert.match(source,/data-responsive-mode/);assert.match(app,/size:compact\?'small':state\.libraryPreferences\?\.cardSize/,'library forwards its selected readable density');
