...
import { freshArchive, loadCloudArchive, saveCloudArchive, saveLocalArchive, type V2ArchiveState, type V2BookRecord } from './archive';
...
function profilesFor(archive: AppArchive): UniverseProfiles {
  if (archive.universes?.empyrean && archive.universes?.prythian) return archive.universes;
  const profiles = freshUniverseProfiles(String(archive.profile.path || 'rider'));
  profiles.empyrean.points = Number(archive.profile.points) || 0;
  profiles.empyrean.rankIndex = Number(archive.profile.rankIndex) || 0;
  profiles.empyrean.onboarded = Boolean(archive.profile.onboarded);
  return profiles;
}
...
export default function CoreFullApp() {
  ...
  const profiles = useMemo(() => profilesFor(archive), [archive]);
  ...
  useEffect(() => {
    ...
    const { data } = supabase.auth.onAuthStateChange((_event, session) => {
      ...
      else loadCloudArchive(nextUser).then((next) => setArchive(next as AppArchive)).catch((reason) => setError(reason instanceof Error ? reason.message : 'Cloud archive unavailable.'));
    });
  ...
  const persistArchive = useCallback(async (next: AppArchive) => {
    const stamped: AppArchive = { ...next, updatedAt: new Date().toISOString() };
    archiveRef.current = stamped;
    setArchive(stamped);
    saveLocalArchive(stamped);
    if (!user) return;
    setSyncState('saving');
    try {
      ...
      setSyncState('error');
      setError(reason instanceof Error ? reason.message : 'Cloud save failed. Your local archive is safe.');
    }
  }, [user]);
  ...
  useEffect(() => {
    function handleWorkspaceDraft(event: Event) {
      ...
      const next: AppArchive = { ...current, books: existing ? current.books.map((book) => book.id === nextBook.id ? nextBook : book) : [...current.books, nextBook], updatedAt: now };
  ...
  const pathRankIndex = rankIndexForPoints(path.id, points);
  const courtRankIndex = prythianRankIndex(points);
  ...
  return <div className="v2-full-app core-path-app" data-path={isPrythian ? undefined : path.id} data-universe={isPrythian ? 'prythian' : 'empyrean'} data-court={isPrythian ? courtId : undefined} style={themeStyle}>
    <aside className="v2-app-sidebar">
      <div className="v2-brand"><span>{isPrythian ? court.glyph : path.glyph}</span><div><small>{isPrythian ? 'The Prythian Archive' : 'The Empyrean Tracker'}</small><strong>{isPrythian ? court.name : path.short}</strong></div></div>
      <nav>{navItems.map((item) => <button key={item.id} className={view === item.id ? 'is-active' : ''} onClick={() => { setProfileBookId(null); setView(item.id); }}><span>{item.icon}</span>{item.label}</button>)}</nav>
      <div className="v2-sidebar-footer"><small>{rank}</small><strong>{displayName}</strong><button onClick={() => signOut().catch((reason) => setError(reason.message))}>Sign Out</button></div>
    </aside>
    <main className="v2-app-main">
      <header className="v2-topbar"><div>{view === 'editor' && <button className="v2-editor-back" onClick={() => setView('library')}>← Back to {isPrythian ? 'Chronicles' : path.copy.navLibrary}</button>}<p>{viewLabel}</p><h1>{view === 'editor' ? 'Book Workspace' : `Welcome back, ${displayName}`}</h1></div><span className={`v2-sync-state is-${syncState}`}>{syncState === 'saving' ? 'Saving…' : syncState === 'saved' ? 'Cloud saved' : syncState === 'error' ? 'Local mode' : 'Cloud ready'}</span></header>
...
function Dashboard({ archive, pathId, isPrythian, courtId, onNavigate, onNewBook, onProfile }: { archive: AppArchive; pathId: PathId; isPrythian: boolean; courtId: PrythianCourtId; onNavigate: (view: AppView) => void; onNewBook: () => Promise<void>; onProfile: (id: string) => void }) {
  ...
  const rank = isPrythian ? PRYTHIAN_RANKS[prythianRankIndex(points)] : path.ranks[rankIndexForPoints(pathId, points)];
...
function Library({ archive, title, addLabel, onNewBook, onEditBook, onProfile, onDeleteBooks, onSaveBooks }: { archive: AppArchive; title: string; addLabel: string; onNewBook: () => Promise<void>; onEditBook: (id: string) => Promise<void>; onProfile: (id: string) => void; onDeleteBooks: (ids: string[]) => Promise<void>; onSaveBooks: (books: V2BookRecord[]) => Promise<void> }) {
  ...
  const selectedBooks = archive.books.filter((book) => selectedIds.includes(book.id));
  const single = selectedBooks.length === 1 ? selectedBooks[0] : null;
...
function Profile({ archive, profiles, onSave }: { archive: AppArchive; profiles: UniverseProfiles; onSave: (next: AppArchive) => Promise<void> }) {
  ...
  const courtId = (profiles.prythian.court || 'night') as PrythianCourtId;
  const court = PRYTHIAN_COURTS[courtId];
  const pathCurrent = rankIndexForPoints(path.id, profile.points || 0);
  const courtCurrent = prythianRankIndex(Number(profile.points) || 0);

  async function changeUniverse(universe: UniverseId) {
    if (busy) return;
    setBusy(true);
    const nextProfiles: UniverseProfiles = { ...profiles, activeUniverse: universe };
    if (universe === 'prythian' && !nextProfiles.prythian.court) nextProfiles.prythian = { ...nextProfiles.prythian, court: 'night' };
    await onSave({ ...archive, universes: nextProfiles });
    setBusy(false);
  }
  async function changeCourt(nextCourt: PrythianCourtId) {
    if (busy) return;
    setBusy(true);
    await onSave({ ...archive, universes: { ...profiles, activeUniverse: 'prythian', prythian: { ...profiles.prythian, court: nextCourt } } });
    setBusy(false);
  }
  async function changePath(nextPath: PathId) {
    ...
    setBusy(true);
    const points = Number(profile.points) || 0;
    await onSave({ ...archive, profile: { ...profile, path: nextPath, rankIndex: rankIndexForPoints(nextPath, points) }, universes: { ...profiles, activeUniverse: 'empyrean', empyrean: { ...profiles.empyrean, path: nextPath, points, rankIndex: rankIndexForPoints(nextPath, points) } } });
    setBusy(false);
  }

  return <div className="v2-profile core-profile"><section className="core-profile-assignment"><div className="prythian-core-selectors"><label>Universe<select value={profiles.activeUniverse} disabled={busy} onChange={(event) => void changeUniverse(event.target.value as UniverseId)}><option value="empyrean">The Empyrean</option><option value="prythian">Prythian</option></select></label>{isPrythian ? <label>Court<select value={courtId} disabled={busy} onChange={(event) => void changeCourt(event.target.value as PrythianCourtId)}>{PRYTHIAN_COURT_IDS.map((id) => <option key={id} value={id}>{PRYTHIAN_COURTS[id].name}</option>)}</select></label> : <label>Path<select value={path.id} disabled={busy} onChange={(event) => void changePath(event.target.value as PathId)}>{PATH_IDS.map((id) => <option key={id} value={id}>{PATHS[id].name}</option>)}</select></label>}</div>{isPrythian ? <><header><div><span>Your court</span><h2>{court.glyph} {court.name}</h2><small>{court.family === 'solar' ? 'Solar Court' : 'Seasonal Court'} · Ruled by {court.ruler}</small></div><div><span>Court standing</span><strong>{PRYTHIAN_RANKS[courtCurrent]}</strong><small>{profile.points || 0} points</small></div></header><div className="core-rank-ladder"><h3>Court standing levels</h3><ol>{PRYTHIAN_RANKS.map((rank,index) => <li key={rank} className={index < courtCurrent ? 'is-unlocked' : index === courtCurrent ? 'is-current' : 'is-locked'}><span>{index+1}</span><div><strong>{rank}</strong><small>{PRYTHIAN_THRESHOLDS[index].toLocaleString()} points</small></div><em>{index < courtCurrent ? 'Unlocked' : index === courtCurrent ? 'Current' : 'Locked'}</em></li>)}</ol></div></> : <><header><div><span>Your assignment</span><h2>{path.glyph} {path.name}</h2></div><div><span>{path.copy.currentRank}</span><strong>{path.ranks[pathCurrent]}</strong><small>{profile.points || 0} points</small></div></header>{profile.abilityName && <article><span>{path.id === 'gryphon' ? 'Mindwork gift' : 'Signet'}</span><strong>{profile.abilityName}</strong><small>{profile.abilityDescription}</small></article>}{profile.creature && <article><span>Bonded creature</span><strong>{profile.creature.name}</strong><small>{profile.creature.color}{profile.creature.tail ? ` ${profile.creature.tail}` : ''}</small></article>}<div className="core-rank-ladder"><h3>{path.progressName} levels</h3><ol>{path.ranks.map((rank,index) => <li key={rank} className={index < pathCurrent ? 'is-unlocked' : index === pathCurrent ? 'is-current' : 'is-locked'}><span>{index+1}</span><div><strong>{rank}</strong><small>{path.thresholds[index].toLocaleString()} points</small></div><em>{index < pathCurrent ? 'Unlocked' : index === pathCurrent ? 'Current' : 'Locked'}</em></li>)}</ol></div></>}</section><section><p>Profile</p><h2>Your archive identity</h2><label>Display name<input value={name} onChange={(event) => setName(event.target.value)} /></label><button onClick={() => void onSave({ ...archive, profile: { ...profile, displayName: name } })}>Save Profile</button></section></div>;
}
